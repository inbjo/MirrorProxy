...
function AdminAuthProviders() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    setNotice(response.ok ? 'Identity provider saved.' : `Unable to save provider: ${(await response.json().catch(() => ({})) as { error?: string }).error ?? 'check the configuration and recent administrator verification'}`)
  ...
  const remove = async (provider: AuthProviderView) => {
    ...
    setNotice(response.ok ? 'Identity provider deleted.' : 'Provider cannot be deleted while user identities are linked.')
  ...
  const test = async (provider: AuthProviderView) => {
    ...
    setNotice(response.ok ? `${provider.display_name} discovery endpoint is reachable.` : `${provider.display_name} connectivity check failed.`)
  }
  const set = <K extends keyof AuthProviderDraft>(key: K, value: AuthProviderDraft[K]) => setDraft((current) => ({ ...current, [key]: value }))
  return <section className="administrator-management identity-provider-settings"><div className="console-section-head"><div><h4>OAuth2 and OpenID Connect</h4><p>Authorization Code, PKCE, one-time state, and verified-email account policy are enforced by the server.</p></div><button onClick={() => setDraft(emptyAuthProvider())}>New provider</button></div><div className="admin-account-list">{providers.map((provider) => <div className="admin-account-row" key={provider.id}><span><strong>{provider.display_name}</strong><small>{provider.kind.toUpperCase()} · {provider.slug} · {provider.enabled ? 'enabled' : 'disabled'}</small></span><span><button onClick={() => test(provider)}>Test</button><button onClick={() => edit(provider)}>Edit</button><button onClick={() => remove(provider)}>Delete</button></span></div>)}</div><form className="provider-form" onSubmit={save}><label>Template<select value={draft.preset} onChange={(event) => chooseTemplate(event.target.value)}>{templates.map((template) => <option value={template.preset} key={template.preset}>{template.display_name}</option>)}</select></label><label>Slug<input required pattern="[a-z0-9-]{2,50}" value={draft.slug} onChange={(event) => set('slug', event.target.value.toLowerCase())} /></label><label>Button label<input required maxLength={80} value={draft.display_name} onChange={(event) => set('display_name', event.target.value)} /></label><label>Protocol<select value={draft.kind} onChange={(event) => set('kind', event.target.value as 'oauth2' | 'oidc')}><option value="oauth2">OAuth2</option><option value="oidc">OpenID Connect</option></select></label><label>Client ID<input required value={draft.client_id} onChange={(event) => set('client_id', event.target.value)} /></label><label>Client secret<input type="password" placeholder={draft.has_client_secret ? 'Saved; leave blank to keep' : 'Required before enabling'} value={draft.client_secret} onChange={(event) => set('client_secret', event.target.value)} /></label>{draft.kind === 'oidc' ? <label className="wide-field">Issuer URL<input required type="url" placeholder="https://id.example.com/realms/company" value={draft.issuer_url ?? ''} onChange={(event) => set('issuer_url', event.target.value || null)} /></label> : <><label className="wide-field">Authorization URL<input required type="url" value={draft.authorization_url ?? ''} onChange={(event) => set('authorization_url', event.target.value || null)} /></label><label className="wide-field">Token URL<input required type="url" value={draft.token_url ?? ''} onChange={(event) => set('token_url', event.target.value || null)} /></label><label className="wide-field">User info URL<input required type="url" value={draft.userinfo_url ?? ''} onChange={(event) => set('userinfo_url', event.target.value || null)} /></label><label className="wide-field">Verified emails URL<input type="url" value={draft.emails_url ?? ''} onChange={(event) => set('emails_url', event.target.value || null)} /></label></>}<label className="wide-field">Scopes<input value={draft.scopes.join(' ')} onChange={(event) => set('scopes', event.target.value.split(/\s+/).filter(Boolean))} /></label>{draft.kind === 'oauth2' ? <><label>Subject field<input value={draft.subject_field} onChange={(event) => set('subject_field', event.target.value)} /></label><label>Email field<input value={draft.email_field} onChange={(event) => set('email_field', event.target.value)} /></label><label>Verified field<input placeholder="Required for automatic linking" value={draft.email_verified_field ?? ''} onChange={(event) => set('email_verified_field', event.target.value || null)} /></label><label>Name field<input value={draft.display_name_field} onChange={(event) => set('display_name_field', event.target.value)} /></label></> : null}<label className="toggle-field"><input type="checkbox" checked={draft.enabled} onChange={(event) => set('enabled', event.target.checked)} />Enable provider</label><label className="toggle-field"><input type="checkbox" checked={draft.allow_registration} onChange={(event) => set('allow_registration', event.target.checked)} />Allow eligible new users</label><label className="toggle-field"><input type="checkbox" checked={draft.auto_link_by_email} onChange={(event) => set('auto_link_by_email', event.target.checked)} />Auto-link verified matching email</label><button className="primary-button" type="submit">{draft.id ? 'Update provider' : 'Add provider'}</button></form>{notice ? <p>{notice}</p> : null}<p className="provider-callback">Callback URL: <code>{window.location.origin}/api/auth/&lt;slug&gt;/callback</code>. Configure the exact public HTTPS URL at the provider.</p></section>
}
...
function BillingGroupRow({ group, reload }: { group: BillingGroupView; reload: () => Promise<void> }) {
