...
- `/os` Debian / Ubuntu / Fedora / Arch Linux / Alpine / OpenWrt / Termux 静态目录代理
...
使用固定 target 路径，例如 `http://127.0.0.1:3000/os/debian/`、`/os/ubuntu/`、`/os/fedora/`、`/os/archlinux/`、`/os/alpine/`、`/os/openwrt/` 或 `/os/termux/`。仅允许这些 target，且每项都有独立可配置 upstream。CLI 可利用 `/os` 基地址生成系统级 APT、DNF 和 pacman 配置文件。
...
本地运行真实客户端协议 smoke（Git、npm/yarn/pnpm、Go、Cargo、pip、CPAN cpanm、RubyGems、Composer）：
