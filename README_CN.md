...
MirrorProxy 是一个基于 Rust 的自部署镜像代理平台。当前可运行切片支持 GitHub 绝对 URL 代理、Composer/Packagist 元数据代理、公开 Docker/OCI registry 拉取代理、npm registry 代理、Go module 代理、Maven Central 代理、RubyGems 代理、Cargo sparse registry 代理，以及 PyPI Simple API 代理，并将 React + Vite + Tailwind Web 控制台内嵌到 Rust 二进制中。
