...
Clojars、Anaconda、LuaRocks、Homebrew bottles 和 Nix 二进制缓存，不依赖执行
包管理器命令。首次写入前会把完整原文件记录到系统原生的用户状态目录
...
首次启动时，MirrorProxy 会创建 SQLite 数据库，初始管理员账号固定为 `admin`，并在本机
启动日志中仅输出一次随机密码。如果 `MIRRORPROXY_ADMIN_PASSWORD` 有值，则会使用该密码但不输出密码。
打开 `/admin`，使用日志中的账号和密码登录。独立管理后台使用作用域限定为 `/admin` 的
`HttpOnly`、`Secure`、`SameSite=Strict` Cookie，因此生产环境必须通过 HTTPS 访问。
数据库只保存 Argon2id 密码哈希；成功和失败登录都会写入审计，连续失败会受到限速并
触发管理员账号短期锁定。

超级管理员可以创建或禁用其他 `admin`、`super_admin` 账号；系统拒绝禁用最后一个
有效超级管理员。禁用账号、修改账号或密码、重置密码都会撤销该账号的会话。管理员可在
安全页面修改自己的账号，修改时还会移除该账号登记的 Passkey。本机应急恢复命令会定位
最初创建的管理员（即使其账号已被修改）、生成并输出新的随机密码，同时解除禁用和锁定状态：
...
密码和 Outbox 邮件正文直接保存在 SQLite 中，API 不返回密码。SQLite Outbox
...
不可用，系统会阻止解绑最后一个外部身份。OAuth Client Secret 直接保存在 SQLite 中，
API、日志和审计不会返回密钥。OAuth/OIDC 控制面请求直连，
...
`GET /admin/api/stats` 返回当前配置月的摘要、配额剩余字节、按日/按 target 的流量
数据及流量最高的十个代理 target，使用管理员 Session Cookie 鉴权。

`POST /admin/api/password` 接收 `current_password` 和至少 12 位的新密码。修改成功后
会撤销全部管理员会话，包括发起修改的当前会话。

`POST /admin/api/username` 接收 `current_password` 和 `new_username`。修改成功后会
撤销该账号的全部会话和已登记 Passkey。
...
[quota]
enabled = true
bidirectional_accounting = false # true 表示每个实际传输字节按两倍计费
monthly_gb = 500
...
当 VPS 厂商同时计算上游流入和客户端流出时，可设置
`bidirectional_accounting = true`（或环境变量
`MIRRORPROXY_QUOTA_BIDIRECTIONAL_ACCOUNTING=true`）。此时向客户端实际传输 1 GiB 会按
2 GiB 计入实例、计费组和用户配额；HTTP 可观测性指标仍保留实际响应字节数。
...
CI 还会设置 `MIRRORPROXY_SMOKE_NATIVE_EXTENDED=1`：先由独立 CLI 写入并精确回滚
LuaRocks、Nix 和 Homebrew 配置，再使用对应原生客户端访问 MirrorProxy，同时通过
`rustup check` 验证 Rustup 发行包和更新端点。此模式要求环境中已安装 `brew`、
`nix`、`rustup` 和 `luarocks`。
...
| 验证层级 | 已验证目标 |
| --- | --- |
| 语言/开发生态原生客户端 | Git、npm、Yarn、pnpm、Go modules、Cargo、pip、CPAN/cpanm、RubyGems、Maven、NuGet、CRAN/R、Cabal/Hackage、LuaRocks（含 CLI 写入配置）、Composer、Rustup、Nix、Homebrew、Docker/OCI |
...
- 持续维护 Prometheus/OpenTelemetry 指标、结构化请求追踪和不记录凭据的告警
  示例，并确保其兼容受支持版本。
- 对所有声明 `local-config` 的目标持续维护包管理器专用改源与精确回滚；依赖命令或
  内部状态数据库的目标明确保留为模板，不误报为可安全写入。
