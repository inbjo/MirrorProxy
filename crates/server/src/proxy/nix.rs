...
pub async fn root(State(state): State<AppState>) -> Result<Response, ProxyError> {
    if !state.config().is_enabled("nix") {
        return Err(ProxyError::Disabled("nix"));
    }
    Response::builder()
        .status(200)
        .header(
        ...
        .body(Body::from("MirrorProxy Nix binary cache proxy\n"))
...
mod tests {
    #[test]
    fn keeps_narinfo_signature_payloads_unmodified() {
        assert!("abc.narinfo".ends_with(".narinfo"));
    }
}
