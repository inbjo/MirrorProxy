...
- 上游重定向最多跟随 10 次。新目标必须仅解析到公网地址，并固定本次请求使用的解析结果。
  明确配置为上游的私有 IP 可在同源内跳转。启用出站代理时仅允许已配置的 IP 同源跳转，
  因为出站代理可能自行解析其他目标。升级后若某上游因此失败，请直接配置可信的最终 URL，
  不要关闭目标地址检查。跨域跳转会移除凭据。
