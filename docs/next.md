...
状态：已完成。已增加普通用户流量控制台、后台用户与计费组管理、默认/自定义/不限量
个人月额度、计费组共享月额度和唯一计费组约束。用户子域请求会在同一个 SQLite 事务中
原子预留全局、计费组与个人三层额度，任一层不足都会回滚整次预留；流式响应完成后按实际
字节同时结算三层统计。换组只影响后续请求。用户可查看今日、本月、请求数、错误数、剩余
额度、按镜像类型统计和近期趋势；管理员可查看用户本月用量、调整组与额度、禁用用户及
轮换子域名。环境变量、Compose 和中英文文档已补齐。工作区共 215 项 Rust 用例，其中
208 项通过，另 7 项本地 listener mock 因受限沙箱 `EPERM` 无法运行；Clippy、前端 10 项
单元测试、生产构建、Compose 和差异检查通过。Playwright 可发现 11 项流程用例，但同样
因当前环境禁止本地监听而未重新执行。
