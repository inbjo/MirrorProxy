...
- chsrc 主要目标现已完成 catalog 登记；当前 CLI 写入/回滚覆盖 npm、pip、cargo、go、composer、docker、apt、Alpine apk、Void xbps、dnf、pacman、Maven、RubyGems、NuGet、CPAN、CRAN、Hackage、Clojars、Anaconda；Guix 会生成官方 `--substitute-urls` 单次命令，其他登记目标明确标为仅配置/计划中。
...
- 配置持久化与管理后台：约 85%。
- CLI 改源能力：约 69%（已覆盖计划中首批目标、Bun、Alpine apk、Void xbps 与常用 Linux 配置并具备回滚；尚未覆盖更多生态细节）。
