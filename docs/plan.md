...
- 仍缺少部分计划中的生态 adapter（部分发行版静态目录等）；Homebrew bottles 已通过 `/homebrew` 提供 GHCR OCI bottle 流式代理。
...
- 代理服务基础能力：约 91%（主流开发生态已覆盖，Homebrew bottles 已接入，并已有主要客户端协议 smoke 与小对象磁盘缓存；部分客户端/adapter 尚缺）。
...
- 对齐 chsrc 支持源范围：约 74%，主要目标已登记且更多语言协议可代理，但 OS/软件仓库 adapter 仍有明显缺口。
- 整体按本计划口径估算：约 91%。
