...
- 仍缺少多个计划中的生态 adapter（Homebrew 与 OS 静态目录等）。
...
- 代理服务基础能力：约 84%（主流开发生态已覆盖，但协议级客户端 smoke、缓存与后续 adapter 尚缺）。
...
- 对齐 chsrc 支持源范围：约 69%，主要目标已登记且更多语言协议可代理，但 OS/软件仓库 adapter 仍有明显缺口。
- 整体按本计划口径估算：约 84%。
